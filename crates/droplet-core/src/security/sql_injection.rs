...
mod tests {
    ...
    fn known_gap_multistmt_injection_via_filter_executes_arbitrary_ddl() {
        ...
        assert!(r.is_ok(), "KNOWN-GAP canary: multi-statement injection via filter WHERE currently succeeds — execute_batch in new_view runs the second DDL (DROP) statement. Flips to is_err() when new_view is fixed to use single-statement execute().");
    }
    ...
    fn known_gap_query_wrap_is_not_parenthesized_multistmt_copy_writes() {
        ...
        assert!(wrote, "KNOWN-GAP canary: query() multi-statement COPY TO a local file currently SUCCEEDS via execute_batch; the CREATE VIEW wrap does not parenthesize agent SQL. Flips to !wrote when local_sql parenthesizes the agent subquery.");
    }
