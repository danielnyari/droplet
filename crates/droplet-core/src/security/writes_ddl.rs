...
mod tests {
    ...
    fn known_gap_multistatement_create_table_executes_write() {
        ...
        assert!(r.is_ok(), "CANARY: currently r.is_ok() (smuggled CREATE TABLE executes) — flip to is_err() when fixed"); assert!(evil_exists, "CANARY: evil table currently persists in session engine — assert false when fixed");
    }
    ...
    fn known_gap_multistatement_attach_database_succeeds() {
        ...
        assert!(r.is_ok(), "CANARY: currently r.is_ok() (smuggled ATTACH executes) — flip to is_err() when fixed");
    }
    ...
    fn known_gap_multistatement_install_extension_runs() {
        ...
        assert!(r.is_ok(), "CANARY: currently r.is_ok() (smuggled INSTALL executes) — flip to is_err() when fixed");
    }
    ...
    fn known_gap_comment_then_semicolon_smuggles_second_statement() {
        ...
        assert!(r.is_ok(), "CANARY: currently r.is_ok() (smuggled CREATE TABLE via comment+; executes) — flip to is_err() when fixed"); assert!(leaked.is_ok(), "CANARY: evil2 table currently exists and is queryable — assert is_err() when fixed");
    }
