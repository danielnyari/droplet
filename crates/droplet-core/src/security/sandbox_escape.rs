...
fn help_and_input_builtins_are_blocked() {
    for (i, code) in ["help(str)", "input()"].iter().enumerate() {
        ...
        assert!(s.run_code(code).is_err(), "interactive I/O builtins must be NameErrors: {code}");
    }
}
...
fn raw_string_replace_bomb_is_bounded_by_limited_tracker() {
    ...
    assert!(r.is_err(), "CANARY (gap closed): string-amplification bomb is bounded by LimitedTracker max_memory — raises MemoryError; got {r:?}");
    assert!(matches!(r.as_ref().unwrap_err(), crate::DropletError::Monty(_)), "breach surfaces as Monty error path, got {r:?}");
    assert_eq!(s.run_code("1+1").unwrap(), MontyObject::Int(2), "session survives");
}
